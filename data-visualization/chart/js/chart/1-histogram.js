...
function drawVerticalBarChart(title, chartAreaWidth, chartAreaHeight, chartData, chartXLabels = [], columnsColor) {
...
function drawHistogramChart(chartAreaWidth, chartAreaHeight, chartData, title, columnsColor) {
    if (!Array.isArray(chartData) || chartData.length == 0) {
        logAndAddError('Invalid chart data. It has to be an array with numerical values.')
        return
    }

    var greatest = d3.max(chartData)
    var lowest = d3.min(chartData)
    var amplitude = greatest - lowest
    var k = Math.round(Math.sqrt(chartData.length))
    var kWidth = amplitude/k
    ...
    var j = 0
    for (i = 0; i < k; i++) {
        var min = (i * kWidth) + lowest
        var max = ((i + 1) * kWidth) + lowest
        distribution[distribution.length] = 0
        chartXLabels[chartXLabels.length] = Math.round(min) + ' - ' + Math.round(max)
        do {
            var value = chartData[j++]
    ...
    drawVerticalBarChart(title, chartAreaWidth, chartAreaHeight, distribution, chartXLabels, columnsColor)

}

function drawHistogramChartFromCSV(filePath, columnPosition, chartAreaWidth, chartAreaHeight, columnsColor) {
    ...
    d3.dsv(';', filePath, null, parse).then(function(data) {
        if (data.length > 0) {
            ...
            drawHistogramChart(chartAreaWidth, chartAreaHeight, values, title, columnsColor)
        } else {
