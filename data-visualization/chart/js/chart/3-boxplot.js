 ...
 */
function drawBoxplotChart(title, chartAreaWidth, chartAreaHeight, orientation, boxes, color) {

    var borderColor = tryToCreateColorObjectFromValue(color)
    borderColor = rgbFromColorObject(borderColor)

    var fillColor   = tryToCreateColorObjectFromValue(color)
    fillColor.a = 0.7
    fillColor = rgbFromColorObject(fillColor)
...
function drawBoxplotChartFromCSV(filePath, columns, orientation, width, height, color) {
        ...
        if (data.length > 0) {
            if (d3.keys(boxes).length > 0) {
                ...
                if (d3.keys(boxes).length == 0) {
                    ...
                    drawBoxplotChart('Box plot chart from ' + filePath, width, height, orientation, boxes, color)
                }
            } else {
