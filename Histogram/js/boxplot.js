...
function drawBoxplotChartFromCSV(filePath, columns, orientation, width, height, orientation) {
    ...
    // orientation
    // boxes 
    // { 'First Column Label': { 'values': [1,2,3,4,5,6,7], 'q1': 2, 'q2': 4, 'q3': 6 , 'min': 1, 'max': 7}, ... }
    ...
    function parse(object) {
        ...
        if (validKeys.length == 0) {
            ...
            columns.forEach(function(column){
                if (column < 0 || column >= keys.length) {
                    logAndAddError('Invalid column value ' + column + '. \nThe column positions must be numerical values from 0 to number of table columns minus 1. For examplo: 0;1;2 .')
                } else {
                    var key = keys[column]
        ...
        validKeys.forEach(function(key){
            ...
            if (!isNaN(valueAsNumber)) {
                if ((typeof boxes[key]) != 'undefined' && Array.isArray(boxes[key].values)) {
    ...
    function filterBoxes() {
        ...
        d3.keys(boxes).forEach(function(key) {
            if (boxes[key].min >= 0) {
                ...
                logAndAddError('The column ' + key + ' has negative values. This implementation does not support negative values yet.')
            }
        })
        boxes = newBoxes
        // boxes = { 
        //     'First Column Label': { 'values': [1,2,3,4,5,6,7,8],'min': 1, 'max': 7},
        //     'Second Column Label': { 'values': [1,2,3,4,5,6,7,30,10], 'min': 1, 'max': 30},
        //     'Third Column Label': { 'values': [1,2,3,4,5,6,7], 'min': 1, 'max': 7}
        // }
    }
    ...
    d3.dsv(';', filePath, null, parse).then(function(data) {
        if (data.length > 0) {
            ...
            defineQuartiles()
            if (boxes.length) {
                logAndAddError('No valid column to plot. The column must contains only positve numerical values (it does not support negative values yet).')
            } else {
